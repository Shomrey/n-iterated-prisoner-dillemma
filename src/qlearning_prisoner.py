...
class QLearningPrisoner(Prisoner):
    prisoner_type = 'q-learning'

    def __init__(self, id):
        super().__init__(id)
        self._prisoner_type = QLearningPrisoner.prisoner_type
        self._last_payoff = 0
        self._next_vote = 'C'
        self._qlearning_model = QLearningModel()

    def strategy(empty = 0):
        return Strategy(lambda id: QLearningPrisoner(id), QLearningPrisoner.prisoner_type)
...
class QLearningModel():
    def __init__(self, learning_rate=0.9, reward_discount=0.9, epsilon=0.9):
    ...
    def run_round(self, actions, last_action, round_payoff):
        ...
        self._current_state_idx = self.get_move_idx(self.calculate_state(actions))
        # choose next action
